...
public class ProjetoAPDA2 {
    ...
    public static void  mergeSort (ArrayList<Times> tabelaDeTimes, int tamVetor){
        int ele = 1;
        int limiteE, meio, limiteD;
        
        while(ele < tamVetor){
            
            limiteE = 0;
            
            while(limiteE + ele < tamVetor){
                meio = limiteE + ele;
                limiteD = limiteE+2*ele;
                
                if(limiteD > tamVetor){limiteD = tamVetor;}
                
                merge(tabelaDeTimes,limiteE,meio,limiteD);
    ...
    public static void merge(ArrayList<Times> tabelaDeTimes, int inicio, int meio, int fim){
        
        ArrayList<Times> tabelaDiv = new ArrayList();
        int i = inicio;
        int m = meio;
        int posicao = 0;
        ...
        while(i<meio&&m<fim){
            ...
            if(tabelaDeTimes.get(i).pontos <= tabelaDeTimes.get(m).pontos){
                tabelaDiv.add(posicao, tabelaDeTimes.get(i)); 
                posicao++;
                i++;
            }
            else{
                tabelaDiv.add(posicao, tabelaDeTimes.get(m)); 
                posicao++;
                m++;
            }
        }
        ...
        for(posicao = 0,i=inicio;i<fim;i++,posicao++){
            tabelaDeTimes.add(i, tabelaDeTimes.get(posicao));
        }
    ...
    public static void carregarTimes(ArrayList<Times> tabelaDeTimes){
        ...
        try{
            ...
            while(scanFile.hasNextLine()){
               ...
               mergeSort(tabelaDeTimes, tabelaDeTimes.size());
