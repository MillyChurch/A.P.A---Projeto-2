...
public class ProjetoAPDA2 {
    ...
    public static int particaoVetor (ArrayList<Times> tabelaDeTimes, int limiteEs, int limiteDi){
    
        Times pivo = tabelaDeTimes.get(limiteEs);
        int inicioVetor = limiteEs+1;
        int fimDoVetor = limiteDi;
        
        while(inicioVetor <= fimDoVetor){
            
            while(inicioVetor <= fimDoVetor && tabelaDeTimes.get(inicioVetor).pontos >= pivo.pontos){
                inicioVetor++;
            }
                    
            while(fimDoVetor >= inicioVetor && tabelaDeTimes.get(fimDoVetor).pontos < pivo.pontos){
                fimDoVetor--;
            }   
            
            if(inicioVetor<fimDoVetor){
                Collections.swap(tabelaDeTimes, fimDoVetor, inicioVetor);
                inicioVetor++;
                fimDoVetor--;
            }
        ...
        Collections.swap(tabelaDeTimes, limiteEs, fimDoVetor);
        return fimDoVetor;
    }
    
    public static void quickSort (ArrayList<Times> tabelaDeTimes, int limiteEs, int limiteDi){
        if(limiteEs < limiteDi){
            int pivo = particaoVetor(tabelaDeTimes, limiteEs, limiteDi);
            quickSort(tabelaDeTimes, limiteEs, pivo-1);
            quickSort(tabelaDeTimes,pivo+1,limiteDi);
        }
    }
    ...
    public static void carregarTimes(ArrayList<Times> tabelaDeTimes){
        ...
        try{
            ...
            while(scanFile.hasNextLine()){
               ...
               quickSort(tabelaDeTimes, 0, tabelaDeTimes.size()-1);
